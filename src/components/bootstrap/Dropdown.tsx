...
export const DropdownToggle: FC<IDropdownToggleProps> = ({ children, isOpen, setIsOpen, hasIcon }) => {
	...
	return (
		<Reference>
			{({ ref }) =>
				cloneElement(
					// @ts-ignore
					children.props.isButtonGroup ? <span className="visually-hidden">Toggle Dropdown</span> : children,
					{
						// @ts-ignore
...
interface IDropdownMenuProps extends HTMLAttributes<HTMLUListElement> {
	isOpen?: boolean;
	setIsOpen?: (value: ((prevState: boolean) => boolean) | boolean | null) => void | null;
	children: ReactElement<IDropdownItemProps> | ReactElement<IDropdownItemProps>[] | ReactNode | ReactNode[];
	className?: string;
...
const ItemWrapper = forwardRef<HTMLLIElement, IItemWrapperProps>(({ children, className, ...props }, ref) => {
	return (
		<li
			ref={ref}
