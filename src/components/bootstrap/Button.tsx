import React, { forwardRef, HTMLAttributeAnchorTarget, HTMLAttributes, ReactElement, ReactNode } from 'react';
...
interface IButtonGroupProps extends HTMLAttributes<HTMLDivElement> {
	children: ReactElement<IButtonProps>[] | ReactElement<IDropdownProps>[] | JSX.Element | JSX.Element[];
	className?: string;
	isToolbar?: boolean;
	isVertical?: boolean;
	size?: 'sm' | 'lg' | null;
...
export interface IButtonProps
	extends HTMLAttributes<HTMLButtonElement | HTMLAnchorElement | HTMLInputElement | HTMLLinkElement> {
	children?: ReactNode;
	...
	className?: string;
	icon?: TIcons;
	rounded?: 'default' | 0 | 1 | 2 | 3 | 'bottom' | 'top' | 'circle' | 'end' | 'start' | 'pill' | null;
	size?: 'sm' | null | 'lg';
	isDisable?: boolean;
...
Button.propTypes = {
	...
	rounded: PropTypes.oneOf(['default', 0, 1, 2, 3, 'bottom', 'top', 'circle', 'end', 'start', 'pill']),
	 ...
	 */
	target: PropTypes.oneOfType([PropTypes.oneOf(['_blank', '_self', '_parent', '_top']), PropTypes.string]),
